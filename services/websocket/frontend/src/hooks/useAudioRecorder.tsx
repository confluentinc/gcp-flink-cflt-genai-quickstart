...
const MAX_RECORDING_DURATION = 30; // 30 seconds maximum recording time
  ...
  const startTimer = () => {
    timerRef.current = setInterval(() => {
      setDuration((prev) => {
        ...
        // Automatically stop recording if we hit max duration
        if (newDuration >= MAX_RECORDING_DURATION) {
          stopRecording();
          toast({
            title: "Recording limit reached",
            description: `Maximum recording time of ${MAX_RECORDING_DURATION} seconds reached.`,
          });
        }
        return newDuration;
      });
  ...
  const startRecording = async () => {
    try {
      ...
      toast({
        ...
        description: `Speak clearly into your microphone (${MAX_RECORDING_DURATION}s max)`,
      });
    } catch (err) {
